...
<li class="text-slate-600 select-none">/</li>
<li class="text-xs text-slate-400">Tambah Nilai</li>
@endsection

<x-layouts.admin-layout>
    <div class="space-y-6">
        <div class="glass-panel rounded-2xl shadow-xl p-6">
            <h2 class="font-display font-bold text-xl text-white tracking-wide">Tambah Data Nilai</h2>
            <p class="text-xs text-slate-400 mt-1">Isi nilai lengkap untuk siswa yang dipilih.</p>
        </div>
                ...
                <form action="{{ route('admin.grades.store') }}" method="POST" class="space-y-6">
                    ...
                    <div class="grid gap-6 lg:grid-cols-3">
                        <div class="space-y-2 lg:col-span-2">
                            ...
                            <select name="student_id" class="w-full rounded-2xl bg-slate-950/50 border border-white/10 px-4 py-3 text-slate-200 text-sm focus:border-rose-500 focus:ring-2 focus:ring-rose-500/20 outline-none transition">
                                ...
                                @foreach($students as $student)
                                    <option value="{{ $student->id }}" {{ old('student_id') == $student->id ? 'selected' : '' }}>{{ $student->nis }} - {{ $student->nama }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="space-y-2 lg:col-span-1">
                            ...
                            <input type="text" name="mata_pelajaran" value="{{ old('mata_pelajaran') }}" class="w-full rounded-2xl bg-slate-950/50 border border-white/10 px-4 py-3 text-slate-200 text-sm focus:border-rose-500 focus:ring-2 focus:ring-rose-500/20 outline-none transition" />
                        ...
                        <div class="space-y-2">
                            ...
                            <input type="number" step="0.01" name="nilai_ujian" value="{{ old('nilai_ujian') }}" class="w-full rounded-2xl bg-slate-950/50 border border-white/10 px-4 py-3 text-slate-200 text-sm focus:border-rose-500 focus:ring-2 focus:ring-rose-500/20 outline-none transition" />
                        ...
                        <div class="space-y-2">
                            ...
                            <input type="number" step="0.01" name="nilai_sekolah" value="{{ old('nilai_sekolah') }}" class="w-full rounded-2xl bg-slate-950/50 border border-white/10 px-4 py-3 text-slate-200 text-sm focus:border-rose-500 focus:ring-2 focus:ring-rose-500/20 outline-none transition" />
                        ...
                        <div class="space-y-2">
                            ...
                            <input type="number" step="0.01" name="nilai_akhir" value="{{ old('nilai_akhir') }}" class="w-full rounded-2xl bg-slate-950/50 border border-white/10 px-4 py-3 text-slate-200 text-sm focus:border-rose-500 focus:ring-2 focus:ring-rose-500/20 outline-none transition" />
                        ...
                        <div class="space-y-2 lg:col-span-3">
                            ...
                            <textarea name="catatan" rows="4" class="w-full rounded-2xl bg-slate-950/50 border border-white/10 px-4 py-3 text-slate-200 text-sm focus:border-rose-500 focus:ring-2 focus:ring-rose-500/20 outline-none transition" placeholder="Catatan tambahan...">{{ old('catatan') }}</textarea>
                    ...
                    <div class="flex flex-wrap gap-3 justify-end">
                        ...
                        <button type="submit" class="px-4 py-2.5 rounded-xl bg-gradient-to-r from-rose-500 to-pink-500 text-white text-xs font-semibold transition shadow-md">Simpan</button>
                    </div>
                </form>
            </div>
        </div>
