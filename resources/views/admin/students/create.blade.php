...
<li class="text-slate-600 select-none">/</li>
<li class="text-xs text-slate-400">Tambah Siswa</li>
@endsection
    ...
    <div class="space-y-6">
        <div class="glass-panel rounded-2xl shadow-xl p-6">
            <div class="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
                <div>
                    <h2 class="font-display font-bold text-xl text-white tracking-wide">Tambah Siswa Baru</h2>
                    <p class="text-xs text-slate-400 mt-1">Tambahkan data siswa lengkap ke periode kelulusan aktif.</p>
                </div>
            </div>
        </div>
                ...
                <form action="{{ route('admin.students.store') }}" method="POST" class="space-y-6">
                    ...
                    <div class="grid gap-6 lg:grid-cols-2">
                        <div class="space-y-2">
                            ...
                            <input type="text" name="nis" value="{{ old('nis') }}" class="w-full rounded-2xl bg-slate-950/50 border border-white/10 px-4 py-3 text-slate-200 text-sm focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 outline-none transition" />
                        ...
                        <div class="space-y-2">
                            ...
                            <input type="text" name="nama" value="{{ old('nama') }}" class="w-full rounded-2xl bg-slate-950/50 border border-white/10 px-4 py-3 text-slate-200 text-sm focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 outline-none transition" />
                        ...
                        <div class="space-y-2">
                            ...
                            <select name="school_class_id" class="w-full rounded-2xl bg-slate-950/50 border border-white/10 px-4 py-3 text-slate-200 text-sm focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 outline-none transition">
                                ...
                                @foreach($classes as $class)
                                    <option value="{{ $class->id }}" {{ old('school_class_id') == $class->id ? 'selected' : '' }}>{{ $class->nama_kelas }} - {{ $class->jurusan }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="space-y-2">
                            ...
                            <select name="graduation_period_id" class="w-full rounded-2xl bg-slate-950/50 border border-white/10 px-4 py-3 text-slate-200 text-sm focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 outline-none transition">
                                ...
                                @foreach($periods as $period)
                                    <option value="{{ $period->id }}" {{ old('graduation_period_id') == $period->id ? 'selected' : '' }}>{{ $period->tahun_ajaran }} - {{ $period->semester }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="space-y-2">
                            ...
                            <input type="number" step="0.01" name="nilai_rata_rata" value="{{ old('nilai_rata_rata') }}" class="w-full rounded-2xl bg-slate-950/50 border border-white/10 px-4 py-3 text-slate-200 text-sm focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 outline-none transition" />
                        ...
                        <div class="space-y-2 flex items-center gap-3">
                            <label class="inline-flex items-center gap-3 rounded-2xl border border-white/10 bg-slate-950/50 px-4 py-3 text-slate-200 text-sm w-full">
                                <input type="checkbox" name="status" value="LULUS" class="h-4 w-4 text-emerald-500 focus:ring-emerald-500" {{ old('status') === 'LULUS' ? 'checked' : '' }} />
                        ...
                        <div class="space-y-2 md:col-span-2">
                            ...
                            <textarea name="catatan" rows="4" class="w-full rounded-2xl bg-slate-950/50 border border-white/10 px-4 py-3 text-slate-200 text-sm focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 outline-none transition" placeholder="Catatan tambahan...">{{ old('catatan') }}</textarea>
                    ...
                    <div class="flex flex-wrap gap-3 justify-end">
                        ...
                        <button type="submit" class="px-4 py-2.5 rounded-xl bg-gradient-to-r from-indigo-500 to-violet-600 text-white text-xs font-semibold transition shadow-md">Simpan</button>
                    </div>
                </form>
            </div>
        </div>
