...
<li class="breadcrumb-item text-sm text-dark active" aria-current="page">Edit Periode</li>
@endsection
    ...
    <div class="card">
        <div class="card-header p-3 pt-2">
            ...
            <div class="text-end pt-1">
                <p class="text-sm mb-0 text-capitalize">Edit Periode Kelulusan</p>
                <h4 class="mb-0">{{ $period->tahun_ajaran }} - {{ $period->semester }}</h4>
            </div>
        </div>
        <div class="card-body">
            <form action="{{ route('admin.graduation-periods.update', $period) }}" method="POST">
                @csrf
                @method('PUT')
                <div class="row">
                    <div class="col-md-6">
                        <div class="input-group input-group-outline my-3">
                            ...
                            <input type="text" name="tahun_ajaran" class="form-control @error('tahun_ajaran') is-invalid @enderror" value="{{ old('tahun_ajaran', $period->tahun_ajaran) }}">
                            @error('tahun_ajaran')
                ...
                <div class="row">
                    <div class="col-md-12">
                        <div class="input-group input-group-outline mb-4">
                            <textarea class="form-control @error('keterangan') is-invalid @enderror" rows="4" name="keterangan" placeholder="Keterangan (opsional)">{{ old('keterangan', $period->keterangan) }}</textarea>
                            @error('keterangan')
                            <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
